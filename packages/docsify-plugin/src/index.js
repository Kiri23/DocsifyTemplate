...
import { addCopyButton, observeCopyButtons } from './adapters/docsify/features/copy-button.js';
import { initSidebarGroups, observeSidebar } from './adapters/docsify/features/sidebar.js';
import { convertMermaidCode, runMermaid } from './adapters/docsify/dom-helpers/mermaid-dom.js';

// Side-effect imports
import './adapters/docsify/features/htmx-virtual.js';
import './adapters/docsify/features/tutorial-header.js';

// ─── Public API ───────────────────────────────────────────────────────────────

export function createPlugin(options = {}) {
  const renderer = options.renderer;
  if (!renderer) throw new Error('[docsify-kiri] createPlugin requires a renderer. Pass { renderer: preactRenderer } or your own.');

  // Register custom components if provided (overrides defaults)
  if (options.components) registerAll(options.components);

  // renderer decides: placeholder (interactive) or string (SSR)
...
// ─── Default: batteries-included with Preact renderer + default components ───
