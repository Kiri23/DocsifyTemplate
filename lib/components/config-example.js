...
window.ConfigExample = function ConfigExample(data) {
  ...
  var codeLines = lines.map(function(line, i) {
    ...
    var escapedLine = U.escapeHtml(line);
  ...
  var titleHtml = title
    ? U.headerBar(title, '<span class="text-gray-500 text-xs">' + annotations.length + ' annotation' + (annotations.length !== 1 ? 's' : '') + '</span>')
    : '';
