...
window.StatusFlow = function StatusFlow(data) {
  ...
  var buttons = states.map(function(state, i) {
    ...
    return '<button id="' + btnId + '" ' +
      ...
      U.escapeHtml(state.label || state.id) +
    '</button>' + arrow;
  }).join('');
  ...
  var panels = states.map(function(state, i) {
    ...
    if (state.trigger) {
      parts.push('<div class="mb-2">' + U.sectionLabel('Trigger:') + ' <span class="text-gray-300 text-sm">' + U.escapeHtml(state.trigger) + '</span></div>');
    }

    if (state.next && state.next.length > 0) {
      parts.push(
        '<div class="mb-2">' + U.sectionLabel('Next states:') + ' ' +
        '<span class="text-sm">' + state.next.map(function(n) {
          return U.typeBadge(n);
        }).join(' ') + '</span></div>'
      );
    }
    ...
    if (state.effects && state.effects.length > 0) {
      parts.push(
        '<div>' + U.sectionLabel('Side effects:') + ' ' +
        '<span class="text-sm">' + state.effects.map(function(e) {
          return '<span class="inline-block bg-cyan-900/25 text-cyan-300 text-xs px-2 py-0.5 rounded-md mr-1">' + U.escapeHtml(e) + '</span>';
        }).join('') + '</span></div>'
      );
    }
  ...
  return U.darkContainer(
    '<div class="flex flex-wrap items-center gap-2 mb-1 p-4">' + buttons + '</div>' +
