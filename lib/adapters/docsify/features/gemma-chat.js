...
function isToolEnabled(toolName) {
  try {
    ...
    if (!configKey) return true; // unknown tools pass through
    return cfg?.features?.chat?.tools?.[configKey] !== false;
  } catch { return true; }
}
...
function getSelectedModelConfig() {
  ...
  return getModelCatalog().find(m => m.id === id) || getModelCatalog()[0];
}
...
export function injectGemmaChat() {
  ...
  createWidget();
  populateModelSelect(getModelCatalog(), (modelId) => {
  ...
  if (savedModel && getModelCatalog().some(m => m.id === savedModel)) {
