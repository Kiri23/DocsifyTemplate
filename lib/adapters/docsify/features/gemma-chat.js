...
import { createChatEngine, MODEL_CATALOG } from '../../../core/chat-engine.js';
import {
...
function onWorkerMessage(e) {
  ...
  switch (status) {
    ...
    case 'ready': {
      ...
      show('gemma-ctx-btn');
      // Model name already shown by picker
      $('gemma-input').focus();
...
function bindEvents() {
  ...
  $('gemma-model-picker').addEventListener('click', () => {
    toggleModelDropdown();
  });

  // Close dropdown on outside click
  document.addEventListener('click', (e) => {
    if (!e.target.closest('#gemma-title')) toggleModelDropdown(false);
  });
...
export function injectGemmaChat() {
  ...
  createWidget();
  populateModelSelect(MODEL_CATALOG, (modelId) => {
    // User picked a different model from header dropdown
    engine.resetMessages();
    clearMessages();
    setTps('');
    hide('gemma-reset-btn');
    hide('gemma-input-area');
    hide('gemma-ctx-btn');
    show('gemma-welcome');
    setDot('');
    loadSelectedModel();
