...
function parseToolCalls(text) {
  // Gemma 4 format: <|tool_call>call:FUNCTION_NAME{...}<tool_call|>
  // Also handle JSON-based format the model might produce
  const calls = [];

  // Pattern 1: Gemma native format
  const nativePattern = /<\|tool_call>call:(\w+)\{([^}]*)\}<tool_call\|>/g;
  let match;
  while ((match = nativePattern.exec(text)) !== null) {
    try {
      // Clean up the Gemma string delimiter format
