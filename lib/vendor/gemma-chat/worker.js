...
async function generate(messages, config) {
  ...
  if (isMultimodal) {
    ...
    // Text-only models: plain string content
    const chatMessages = messages.map(m => ({ role: m.role, content: m.content }));
    const prompt = currentProcessor.apply_chat_template(chatMessages, templateOpts);
    inputs = currentProcessor(prompt, { add_special_tokens: false, return_tensor: true });
  }
...
async function load(config) {
  ...
  try {
    ...
    const warmupMsg = isMultimodal
      ? [{ role: "user", content: [{ type: "text", text: "hi" }] }]
      : [{ role: "user", content: "hi" }];
    const warmupOpts = { add_generation_prompt: true };
    if (isMultimodal) warmupOpts.enable_thinking = false;

    const prompt = processor.apply_chat_template(warmupMsg, warmupOpts);
    const inputs = isMultimodal
      ? await processor(prompt, null, null, { add_special_tokens: false })
      : processor(prompt, { add_special_tokens: false, return_tensor: true });
