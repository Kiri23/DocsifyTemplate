...
async function generate(messages, config) {
  ...
  // Only pass tools if model supports native tool calling
  if (config && config.nativeToolCalling) {
    templateOpts.tools = TOOLS;
  }

  const prompt = currentProcessor.apply_chat_template(chatMessages, templateOpts);

  const inputs = await currentProcessor(prompt, null, null, {
  ...
  const streamer = new TextStreamer(currentProcessor.tokenizer, {
    skip_prompt: true,
