...
(function () {
  ...
  const SYSTEM_PROMPT = `You are a helpful documentation assistant with tool-calling capabilities. You can answer questions AND take actions.
...
When the user asks you to DO something (export, navigate, create, scroll), use a tool call. When they ask a QUESTION, answer from the documentation context. Be concise and direct.`;
  ...
  const toolExecutors = {
    export_page: async ({ format }) => {
      ...
      // Find and trigger the export select + button
      const select = document.querySelector('.latex-export-select');
      const btn = document.querySelector('.latex-export-btn');
      if (!select || !btn) return { success: false, message: 'Export UI not found on this page' };
    ...
    generate_component: async ({ component, description }) => {
      // Generate YAML for the requested component
      const yamlExamples = {
    ...
    for (const call of calls) {
      ...
      if (executor) {
        try {
          ...
          results.push({ name: call.name, success: false, message: e.toString() });
          appendActionBubble(call.name, call.args, { success: false, message: e.toString() });
        }
      } else {
        results.push({ name: call.name, success: false, message: `Unknown tool: ${call.name}` });
      }
    }
    return results;
  }

  function appendActionBubble(toolName, args, result) {
    ...
    div.innerHTML = `
      ...
      ${result.message ? `<div class="gemma-action-result">${escapeHtml(result.message)}</div>` : ''}
    `;
